...
def load_file_in_chunks(path):
    ...
    while True:
        ...
        response = warp_pb2.Bytes(chunk=bytes.get_data())
        yield response
...
class FileSender(GObject.Object):
    ...
    def update_progress(self, size_read=0, finished=False):
        ...
        if ((now - self.last_update_time) > self.UPDATE_FREQ) or finished:
...
class FileReceiver(GObject.Object):
    ...
    def receive_data(self, s):
        ...
        if s.file_type == FileType.DIRECTORY:
            ...
            absolute_symlink_target_path = os.path.join(self.save_path, s.symlink_target)
